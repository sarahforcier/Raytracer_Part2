...
glm::vec3 Integrator::TraceRay(Ray r, int depth) const
{
    ...
    if (inter.t < 0.f && depth > 0.f) {
        ...
        if (in != 0.0 && out != 0.0) {
            ...
            Ray refa = Ray(inter.point - 0.00001f * N, rfa);
            refract_color = TraceRay(refa, depth - 1);

            // total internal reflection or reflection
            if (sin2ThetaT > 1.f || inter.object_hit->material->reflectivity > 0.f) {
                glm::vec3 ori = inter.point + 0.00001f * N;
                Ray refe = Ray(ori,glm::normalize(glm::reflect(I,N)));
                reflect_color = TraceRay(refe, depth - 1);
                if (sin2ThetaT > 1.f) {
                    refract_color = reflect_color;
                }
            }
            color = bcolor * glm::mix(refract_color, reflect_color, inter.object_hit->material->reflectivity);
